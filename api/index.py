...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if ext in [".jpg", ".jpeg"]:
            ...
            encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), os.environ.get('JPGE_QUALITY')]
            img_type = "image/jpeg"
        elif ext == ".webp":
            # 设置webp格式图片的压缩质量（数字越大质量越高，最大为100）
            encode_param = [int(cv2.IMWRITE_WEBP_QUALITY), os.environ.get('WEBP_QUALITY')]
            img_type = "image/webp"
        elif ext == ".png":
            # 设置png格式图片的压缩质量（数字越大质量越高，最大为9）
            encode_param = [int(cv2.IMWRITE_PNG_COMPRESSION), os.environ.get('PNG_COMPRESSION')]
            img_type = "image/png"
        else:
